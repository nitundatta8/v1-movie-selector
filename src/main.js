...
$(document).ready(function () {
  ...
  $("#showFMovieList").click(function(){
    ...
    $("#fMovieList").show();
    $('#results').text("Your Movie List is empty");
  })
  ...
  $("#fMovieList").on("click", "button", function () {
    $("#fMovieList").html('');
    
    let movieDelete = movieObj.deleteMovieById(this.id);
    ...
    function getElements(response) {
      if (response) {
        ...
        for (let i = 0; i < response.results.length; i++) {
          ...
          htmlInfo = `<h5><a id="${response.results[i].id}" href="#">${response.results[i].original_title}</h5>
                  <div class="card">    
  ...
  function getSmilarMovie(similarMovie) {
    ...
    if (similarMovie) {
      ...
      for (let i = 0; i < similarMovie.results.length; i++) {
        console.log("val3   ----");
        similarMovieInfo = `<h5><a id="${similarMovie.results[i].id}" href="#"></h5>
                <div class="card"style="width: 10rem;"> 
                  <p>${similarMovie.results[i].original_title}</p>   
          ...
          <div class="card">
            <div class="card">
              <div class="card-body">
                ...
                <button type="button" class="btn btn-primary" id="${movie.id}">Remove from your Favorites</button>
              </div>
            </div>
          </div>
        </div>
